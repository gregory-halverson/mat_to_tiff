...
def read_csv_matrix(csv_filename):
    csv_matrix = numpy.ma.masked_invalid(genfromtxt(csv_filename, delimiter=','))
    numpy.ma.set_fill_value(csv_matrix, numpy.nan)
...
def csv_matrix_to_land_tile_geotiff(csv_filename, matlab_variable_name, tiff_filename):
    sinusoidal_tile_corners = load_sinusoidal_tile_corners()
    hv = os.path.basename(csv_filename).split('_')[-1].split('.')[0]
    x_min = sinusoidal_tile_corners[hv]['x_min']
    y_max = sinusoidal_tile_corners[hv]['y_max']
    matrix = read_csv_matrix(csv_filename, matlab_variable_name)
    matrix[numpy.isnan(matrix)] = 0
    matrix_to_land_tile_geotiff(matrix, x_min, y_max, tiff_filename)

def csv_to_global_tiff(csv_directory, variable_name, output_directory):
    mat_file_list = glob(csv_directory + '/*.mat')
    ...
    for mat_filename in mat_file_list:
        ...
        csv_matrix_to_land_tile_geotiff(mat_filename, variable_name, tiff_filename)
